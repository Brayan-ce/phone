...
export default function Contenido({ categoriaActiva = "nuevos" }) {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      const [prods, cfg] = await Promise.all([
        getProductosConColores(categoriaActiva),
        getConfigHero(),
      ]);
      setProductos(prods);
      setConfig(cfg);
      ...
      <section className={styles.heroBanner}>
        <div className={styles.heroContent}>
          ...
          <p className={styles.heroSub}>
            {config?.hero_subtitulo || "Stock disponible para entrega inmediata."}
          </p>
        ...
        <div className={styles.toolbar}>
          <div className={styles.toolbarLeft}>
            <p className={styles.resultsCount}>
              <strong>{filtrados.length}</strong> productos encontrados
            </p>
          </div>
